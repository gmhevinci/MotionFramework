...
namespace MotionFramework.Window
{
	public abstract class CanvasWindow : UIWindow
	{
		...
		protected override void OnAssetLoad(GameObject go)
		{
			...
			if (_manifest == null)
			{
				MotionLog.Error($"Not found {nameof(UIManifest)} in window {WindowName}");
				return;
			}
			...
			if (_canvas == null)
			{
				MotionLog.Error($"Not found {nameof(Canvas)} in window {WindowName}");
				return;
			}
			...
			if (_raycaster == null)
			{
				MotionLog.Error($"Not found {nameof(GraphicRaycaster)} in window {WindowName}");
				return;
			}
